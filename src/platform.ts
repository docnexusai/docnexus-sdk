 ...
 * Base URLs for docnexus-link and advanced-search are stored in the package; client only passes apiKey and payload.
 *
 * @example
 * import { createPlatformClient, call } from '@docnexus/api-client';
 *
 * const client = createPlatformClient({ apiKey: 'dnx_xxx' });
...
import { getBaseUrlForEndpoint } from "./config";
...
export async function call<T = unknown>(options: PlatformCallOptions): Promise<T> {
  const { apiKey, endpointName, payload, fetch: fetchFn = globalThis.fetch } = options;
  const base = getBaseUrlForEndpoint(endpointName);
 ...
 * Base URLs are taken from package config; client only provides apiKey.
 */
export function createPlatformClient(config: PlatformClientConfig) {
  const { apiKey, fetch: fetchFn } = config;
  return {
    /**
    ...
    call<T = unknown>(endpointName: string, payload?: Record<string, unknown>): Promise<T> {
      return call<T>({ apiKey, endpointName, payload, fetch: fetchFn });
    },
